...
import { buildElectronProxyConfig, resolveSystemProxy, trashItem } from '@/shared/electron'
...
export const listenerElectronEvent = () => {
  app.on('session-created', (session) => {
    const proxy = buildElectronProxyConfig(appState.proxy.host, appState.proxy.port, appState.proxy.mode == 'system')
...
const listenerAppEvent = () => {
  ...
  const handleProxyChange = async () => {
    const changeId = ++proxyChangeId
    let host: string
    let port: string
    let mode: typeof appState.proxy.mode
    let electronProxy: Electron.ProxyConfig
    if (appState.appSetting['network.proxy.enable'] && appState.appSetting['network.proxy.host']) {
      ...
      mode = 'fixed'
      electronProxy = buildElectronProxyConfig(host, port)
    } else if (appState.appSetting['network.proxy.followSystem']) {
      const systemProxy = await resolveSystemProxy().catch((err: Error) => {
        console.log(err)
        return null
      })
      ...
      mode = 'system'
      electronProxy = buildElectronProxyConfig(host, port, true)
    ...
    if (changeId != proxyChangeId) return
    if (appState.proxy.host == host && appState.proxy.port == port && appState.proxy.mode == mode) return
    appState.proxy.host = host
    appState.proxy.port = port
    appState.proxy.mode = mode
    appEvent.proxy_changed(host, port, electronProxy)
  }
  appEvent.on('updated_config', (keys, setting) => {
    ...
    if (
      ...
      keys.includes('network.proxy.followSystem') ||
      ((appState.appSetting['network.proxy.enable'] || appState.appSetting['network.proxy.followSystem']) &&
  ...
  appEvent.on('inited', () => {
    try {
      void app.setProxy(buildElectronProxyConfig(appState.proxy.host, appState.proxy.port, appState.proxy.mode == 'system'))
    } catch {}
    void handleProxyChange()
