...
export interface ListMusicInfoNameSingerQuery {
  list_id: string
  name: string
  singer: string
}
...
/**
 * 创建根据列表Id + 歌名 + 歌手查询音乐信息语句(用于跨源识别同一首歌是否已收藏)
 */
export const createMusicInfoByListAndNameSingerQueryStatement = () => {
  return dbPrepare<ListMusicInfoNameSingerQuery, MusicInfo>(`SELECT "id", "name", "singer", "is_local", "interval", "meta"
    FROM "main"."my_list_music_info"
