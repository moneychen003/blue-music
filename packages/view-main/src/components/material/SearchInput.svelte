<script lang="ts">
  ...
  const showList = () => {
    ...
    listStyle = list.length
      ? `height: ${domList?.scrollHeight ?? 0}px; max-height: ${maxHeight};`
      : `height: 0; max-height: ${maxHeight};`
  }
  const hideList = () => {
    isShowList = false
  ...
  export const setList = (_list: ListItem[]) => {
    ...
    void tick().then(() => {
      if (isShowList) {
        listStyle = list.length
          ? `height: ${domList?.scrollHeight ?? 0}px; max-height: ${document.body.clientHeight * 0.6}px;`
          : `height: 0; max-height: ${document.body.clientHeight * 0.6}px;`
      } else if (list.length) {
        showList()
      }
    })
...
<style lang="less">
  ...
  .content {
    ...
    .list-content {
      ...
      overflow: auto;
      font-size: 13px;
      background-color: color-mix(in srgb, var(--color-main-background) 92%, white 8%);
      border-radius: 8px;
